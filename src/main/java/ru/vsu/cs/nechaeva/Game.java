...
public class Game {
    ...
    // Массив, представляющий мины игрока и компьютера
    public int playerMines[]; // Мины игрока
    public int compMines[]; // Мины компьютера
    /**
     * Процесс игры
    ...
    public boolean setMine(int i, int j, boolean isPlayer) {
        if (isPlayer) {
            for (int k = 0; k < playerMines.length; k++) {
                if (playerMines[k] == 0) { // если ячейка свободна
                    playerMines[k] = (i * 10) + j; // сохраняем координаты мины
                    return true;
                }
            }
        }
        return false; // если не удалось установить мину
    }

    // Метод для проверки попадания на мину
    public void checkMine(int i, int j, boolean isPlayer) {
        int[] mines = isPlayer ? compMines : playerMines;
        for (int mine : mines) {
            if (mine != 0) {
                int mineI = mine / 10;
                int mineJ = mine % 10;
                if (mineI == i && mineJ == j) {
                    // Попадание на мину
                    hitRandomEnemyShip(isPlayer); // Наносим случайный удар по кораблю противника
                    return;
                }
            }
        }
    ...
    // Метод для случайного нанесения урона по кораблю
    private void hitRandomEnemyShip(boolean isPlayer) {
        int[][] enemyMas = isPlayer ? masComp : masPlay; // Определяем, на кого атаковать
        while (true) {
            int i = (int) (Math.random() * 10);
            int j = (int) (Math.random() * 10);
            if (enemyMas[i][j] > 0 && enemyMas[i][j] < 8) { // только если это неповрежденный корабль
                enemyMas[i][j] += 7; // Наносим урон
                testUbit(enemyMas, i, j); // Проверяем на убит ли это корабль
                break;
            }
        }
    }
